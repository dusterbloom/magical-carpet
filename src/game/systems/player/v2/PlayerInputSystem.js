...
/**
 * Handles player input from keyboard, mouse, touch, and motion sensors
 */
export class PlayerInputSystem extends System {
  constructor(engine) {
    super(engine, 'playerInput');
    this.requireDependencies(['playerState', 'playerPhysics']);
    ...
    this.motionSensitivity = {
      pitch: 0.04,  // Controls up/down movement
      yaw: 0.06     // Controls left/right turning
    };
  ...
  async _initialize() {
    // Set mobile flag based on engine input
  ...
  _update(delta) {
    ...
    const physics = this.engine.systems.playerPhysics;
    ...
    // Gentler strafing
    let strafeForce = 0;
    if (input.isKeyDown('KeyA')) strafeForce -= 0.3;
    if (input.isKeyDown('KeyD')) strafeForce += 0.3;
  ...
  setupInput() {
    ...
    input.on('mousemove', (event) => {
      if (input.pointerLocked && this.engine.systems.playerState.localPlayer) {
