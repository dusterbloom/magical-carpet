...
export class StarsParticleSystem {
  ...
  async initialize() {
    // Create two star systems - one for regular stars and one for special KISS stars
  ...
  createRegularStars() {
    ...
    for (let i = 0; i < 3000; i++) {
      ...
      const phi = Math.random() * Math.PI * 0.4; // Increased to 40% of hemisphere
  ...
  createKISSStars() {
    ...
    const gridSize = 130; // Increased size for better visibility
    const patternScale = 250;
    // Position the KISS pattern higher in the sky for better visibility from all angles
    const patternOffset = [-7 * gridSize / 2, 2000, -4000];
